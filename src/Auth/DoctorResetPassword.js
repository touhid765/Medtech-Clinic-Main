...
import './DoctorResetPassword.css'; // Add your CSS file if needed
...
const DoctorResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${URL}/api/auth/doctor-reset/${token}`, {
        password: formData.password,
      });

      if (response.data.success) {
        setSuccess('Password reset successfully. Redirecting to login page...');
        setTimeout(() => navigate('/doctor-login'), 3000); // Redirect to login after 3 seconds
      } else {
